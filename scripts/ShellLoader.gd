...
func LoadShells():
	...
	if (!roundManager.shellLoadingSpedUp): await GlobalVariables.tree.create_timer(.8, false).timeout
	...
	for i in range(numberOfShells):
		...
		if(roundManager.shellLoadingSpedUp): await GlobalVariables.tree.create_timer(.17, false).timeout
		else: await GlobalVariables.tree.create_timer(.32, false).timeout
		pass
	animator_dealerHandRight.play("RESET")
	dealerAI.Speaker_HandCrack()
	if (roundManager.shellLoadingSpedUp): await GlobalVariables.tree.create_timer(.17, false).timeout
	else: await GlobalVariables.tree.create_timer(.42, false).timeout
	...
	if (roundManager.roundArray[roundManager.currentRound].hasIntroductoryText):
		...
		await GlobalVariables.tree.create_timer(3.7, false).timeout
